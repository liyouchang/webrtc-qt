...
int MEDIA_Audio_isTalking(void);						//返回是否正在对讲的状态。
...
int MEDIA_Audio_SoundFromFile(char *pFileName);			//输出声音文件，pFileName为声音文件的绝对路径
int MEDIA_Jpeg_Generate(int iChn,int iJpegNum,int iJpegGop,MEDIA_JPEG_CALLBACK jpeg_callback);//生成iJpegNum张图片，间隔iJpegGop帧生成一张，并且将生成的图片同过jpeg_callback回调给调用者

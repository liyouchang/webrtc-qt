...
static int gb2312_mbtowc (unsigned int *pwc, const unsigned char *s, int n)
{
...
static int gb2312_wctomb (unsigned char *r, unsigned int wc, int n)
{
	if (n >= 2) {
...
static int utf8_mbtowc (unsigned int *pwc, const unsigned char *s, int n)
{
	unsigned char c = s[0];
	if (c < 0x80) {
		*pwc = c;
		...
		return -1; //return code if invalid
	}
	else if (c < 0xe0) {
		if (n < 2)
			return -2; //Return code if only a shift sequence of n bytes was read.
		if (!((s[1] ^ 0x80) < 0x40))
			return -1;
...
static int isvalid (unsigned char *s, int n)
{
	int i;
	for(i = 1; i < n; i++)
	{
...
static int utf8_wctomb (unsigned char *r, unsigned int wc, int n)
{
	int count;
	if (wc < 0x80)
		count = 1;
...
static int changewordu2g (unsigned char *s, unsigned char *inbuf, int n)
{
	unsigned int out2[10];
	utf8_mbtowc(out2, inbuf, n);
	gb2312_wctomb(s, out2[0], 2);
	s[0] |= 0x80;
	s[1] |= 0x80;
	return 1;
}

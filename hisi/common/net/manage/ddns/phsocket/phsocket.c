...
//	if (mysockaddr_in.sin_addr.s_addr == INADDR_NONE)
//	{
//		if (gethostbyname_my(lpszHostAddress,(char *)&mysockaddr_in.sin_addr.s_addr)==0)
//		{
//			if (nAddressIndex)
//				*nAddressIndex = 0;
//			printf("nAddressIndex %d\n",*nAddressIndex);
//			if (szSelectedAddress)
//			{	
//				strcpy(szSelectedAddress,inet_ntoa(mysockaddr_in.sin_addr));
//				printf("%s:%d %s\n",__FUNCTION__,__LINE__,inet_ntoa(mysockaddr_in.sin_addr));
//			}
//		}
//		else
//			return FALSE;
...
BOOL phCreate(int *m_hSocket, unsigned short nSocketPort, int nSocketType, char * lpszSocketAddress)
{
	...
	*m_hSocket = socket(PF_INET, nSocketType, 0);
	...
	t.tv_sec = 15;
	t.tv_usec = 0;
	setsockopt(*m_hSocket, SOL_SOCKET, SO_RCVTIMEO, &t, sizeof(t));

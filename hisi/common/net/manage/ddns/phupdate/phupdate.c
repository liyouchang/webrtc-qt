...
unsigned int KeyMD5Encode(unsigned char* szEncoded, const unsigned char* szData, unsigned int nSize, unsigned char* szKey, unsigned int nKeyLen)
{
    ...
    UTILITY_MD5(firstPad, midResult + PADLEN, PADLEN + nSize);

    memcpy(midResult, opad, PADLEN);
    UTILITY_MD5(midResult, szEncoded, PADLEN + 16);
    return 16;
}
...
// 		LOG(1) (("ExecuteUpdate %s\n"),buffer);
        ...
        if (i < 10)
        {
            //strcpy(szOKDomains[i], domains[i]);
//			memcpy(&szOKDomains[i],&domains[i], 32);
//			printf("[%s]: ssltest %s %d\n",__FUNCTION__,szOKDomains[i],i);
        }
    }
//	Domainlist = i;
//	printf("[%s]: Domainlist=%d\n",__FUNCTION__,Domainlist);
    memset(buffer, 0, 128);
    len = phReadOneLine(phglobal->m_tcpsocket, buffer,sizeof(buffer));
    if (len <= 0)
    {
...
int phddns_step(PHGlobal *phglobal)
{
    ...
    if (phglobal->bNeed_connect)
    {
        ...
        {
            printf("[%s]:InitializeSockets failed, waiting for 5 seconds to retry...\n", __FUNCTION__);
        ...
        if (ret == okDomainsRegistered)
        {
            ...
            printf("[%s]:ExecuteUpdate OK, BeginKeepAlive!\n", __FUNCTION__);
...
// 			LOG(1) ("ExecuteUpdate failed, waiting for 30 seconds to retry...\n");
    ...
    {
        if ((phglobal->nUserType == 3) && (time(0) - phglobal->tmLastSend > 15))
        {

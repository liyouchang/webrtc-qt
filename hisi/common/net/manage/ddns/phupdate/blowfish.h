...
typedef struct {
	unsigned long S[4][256],P[18];
 	...
 	extern void InitBlowfish(blf_ctx *c, const unsigned char* keybuf,unsigned int len);
...
// 	//Remember the src length must 8 mod 0,that is round to 64 bits.
 	extern long Blowfish_EnCode(blf_ctx *c,const char* src,char* dest,unsigned long len) ;
 	extern long Blowfish_DeCode(blf_ctx *c,const char* src,char* dest,unsigned long len) ;

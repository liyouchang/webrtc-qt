...
import com.video.play.AudioThread;
...
public class TestPlayerActivity  extends Activity implements OnClickListener  {
...
//		}
		...
		// 录像弹出框
		infoView = getLayoutInflater().inflate(R.layout.player_video_record_view, null);
	...
	private void initData() {
		mContext = TestPlayerActivity.this;
		...
		if (Utils.checkSDCard()) {
			...
			fileInfo.fileCachePath = videoPath1 + File.separator + "20140428105803.h264";
...
//		public File videoCachePath = null; // 文件夹的缓存路径
		public String fileCachePath = null; // 当前文件的缓存路径
		public int fileReadSpeed = 0;
	}
				...
				try {
					...
					if (randomFile != null) {
						...
						if ((frameType & 0x5F) < 30) {
							...
							if ((readSize <= 0) || (readSize > frameLen)) {
								// 读错数据重新读取
								Thread.sleep(100);
								if (!filePointerMoveToValidPosition(randomFile, 0)) {
									if (!filePointerMoveToValidPosition(randomFile, 1)) {
		...
		public void run() {
			...
			while (runFlag) {
				try {
					...
					if (timeCount >= 10) {
						timeCount = 0;
						sendHandlerMsg(handler, SEEKBAR_REFRESH, fileInfo.readFileSize);
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
...
//				audioThread.stopAudioThread();
			...
			//删除缓存文件
//			Utils.deleteFile(fileInfo.videoCachePath);
		} catch (Exception e) {

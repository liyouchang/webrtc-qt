...
public:
    ...
    AsynDealer(talk_base::Thread *thread = NULL);
    ~AsynDealer();
    ...
    bool SendRecv(const std::string & addr, const std::string & sendData,
...
protected:
    ...
    void recv_z();
    bool SendRecv_z(const std::string & addr,const std::string & sendData,
                    std::string * recvData,int timeout);

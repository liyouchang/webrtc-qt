...
bool JniUtil::JniSendToPeer(const char* peer_id, const char* message) {
	...
	jmethodID mid = g_env_->GetMethodID(callBackCls,"SendToPeer","(Ljava/lang/String;Ljava/lang/String;)V");
	if(!mid){
	...
	g_env_->CallVoidMethod(callBackCls,mid,jni_pid,jni_msg);
	return true;
}
...
bool JniUtil::JniRecvVideoData(const char* peer_id, const char* data, int len) {
	jclass callBackCls = g_env_->FindClass(call_class_name_.c_str());
		...
		jmethodID mid = g_env_->GetMethodID(callBackCls,"RecvVideoData","(Ljava/lang/String;[B)V");
		if(!mid){
			LOGE("get method SendToPeer error");
			return false;
		}
...
bool JniUtil::JniRecvAudioData(const char* peer_id, const char* data, int len) {
	jclass callBackCls = g_env_->FindClass(call_class_name_.c_str());
		...
		jmethodID mid = g_env_->GetMethodID(callBackCls,"RecvAudioData","(Ljava/lang/String;[B)V");
		if(!mid){
			LOGE("get method SendToPeer error");
			return false;
		}

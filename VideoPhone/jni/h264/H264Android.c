...
jint Java_com_video_play_VideoPlayView_initDecoder(JNIEnv* env, jobject thiz, jint width, jint height)
{
	iWidth = width;
	iHeight = height;
	CreateYUVTab_16();
...
jint Java_com_video_play_VideoPlayView_uninitDecoder(JNIEnv* env, jobject thiz)
{
	avcodec_close(c);
	av_free(c);
	av_free(picture);
...
jint Java_com_video_play_VideoPlayView_decodeNalu(JNIEnv* env, jobject thiz, jbyteArray in, jint nalLen, jbyteArray out)
{
	int i;
	int imod;
	int got_picture;
